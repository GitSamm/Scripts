...
ax.plot(r_0/R, np.abs(analyticTot-totalNoise/np.sqrt(f))/analyticTot, marker='D', label='', color='k')
ax.set_yscale('log')
ax.set_xscale('log')
...
#ax.set_title('F. silica total noise diff', fontsize=myTitleSize, color='k', y=1.04)
...
fig.tight_layout()
pdf = PdfPages('VaryBeamNoiseTotDiff.pdf')
pdf.savefig()
pdf.close()
